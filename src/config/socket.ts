// Updated socket.ts with TODOs implemented
...
export class SocketManager {
    ...
    private streamViewers: Map<string, Set<string>> = new Map(); // streamId -> Set<socketId>
    ...
    private handleStreamEvents(socket: Socket) {
        ...
        socket.on('stream:status', async (data: { streamId: string; status: string }) => {
            ...
            if (!stream || typeof stream.vendorId !== 'string' || stream.vendorId !== socket.vendorId) {
    ...
    private handleChatEvents(socket: Socket) {
        ...
        socket.on('chat:delete', async (data: { messageId: string; streamId: string }) => {
            if (!socket.isAuthenticated) return;
    ...
    private handleEcommerceEvents(socket: Socket) {
        socket.on('product:interest', async (data: {
            streamId: string;
            productId: string;
        ...
        socket.on('order:subscribe', async (data: { orderId: string }) => {
            ...
            try {
                ...
                if (order.user_id !== socket.userId && order.vendor_id !== socket.vendorId) {
    ...
    private async updateStreamViewerCount(streamId: string) {
        ...
        try {
            ...
            logger.error('Failed to update stream viewer count in database', error as Error);
        }

        // Cache viewer count in Redis
        try {
            const redisClient = getRedisClient();
            await redisClient.setEx(`stream:${streamId}:viewers`, 60, viewerCount.toString());
        } catch (error) {
    ...
    public async broadcastStreamEvent(streamId: string, event: string, data: any) {
