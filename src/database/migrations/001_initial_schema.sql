...
CREATE TABLE comments (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4 (),
    user_id UUID NOT NULL REFERENCES users (id) ON DELETE CASCADE,
    commentable_type VARCHAR(20) NOT NULL CHECK (
        commentable_type IN ('video', 'stream')
    ),
...
CREATE TABLE likes (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4 (),
    user_id UUID NOT NULL REFERENCES users (id) ON DELETE CASCADE,
    likeable_type VARCHAR(20) NOT NULL CHECK (
        likeable_type IN (
            'video',
            'stream',
            'comment',
...
CREATE INDEX idx_video_jobs_created   ON video_processing_jobs(created_at);
...
CREATE INDEX idx_comments_commentable ON comments (
    commentable_type,
    commentable_id
);
...
CREATE TABLE IF NOT EXISTS media_files (
    ...
    thumbnail_url TEXT,
    metadata JSONB,
    status VARCHAR(20) DEFAULT 'uploading' CHECK (
        status IN (
            'uploading',
            'processing',
            'ready',
