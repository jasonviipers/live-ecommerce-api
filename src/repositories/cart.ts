...
export class CartRepository {
  ...
  static async addItem(
    ...
    return withTransaction(async (client: PoolClient) => {
      ...
      `;
      const checkResult = await client.query(checkSql, [cartId, productId, variantId]);
      ...
      if (checkResult.rows.length > 0) {
        ...
        const updateSql = `
          ...
          RETURNING *
        `;
        const updateResult = await client.query(updateSql, [newQuantity, price, existingItem.id]);
        ...
        const insertSql = `
          ...
          RETURNING *
        `;
        const insertResult = await client.query(insertSql, [cartId, productId, variantId, quantity, price]);
  ...
  static async mergeSessionCartToUser(sessionId: string, userId: string): Promise<Cart> {
    return withTransaction(async (client: PoolClient) => {
      ...
      const sessionItemsResult = await client.query(sessionItemsSql, [sessionCart.id]);
      ...
      for (const sessionItem of sessionItemsResult.rows) {
        ...
        if (existingItemResult.rows.length > 0) {
          ...
          const newQuantity = parseInt(existingItem.quantity) + parseInt(sessionItem.quantity);
