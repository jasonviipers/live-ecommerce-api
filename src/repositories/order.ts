...
export class OrderRepository {
  static async create(data: CreateOrderData): Promise<Order> {
    return withTransaction(async (client: PoolClient) => {
      ...
      for (const item of data.items) {
        ...
        let variantSku = null;
        if (item.variantId) {
          const variantSql = 'SELECT name, sku FROM product_variants WHERE id = $1';
  ...
  static async updatePaymentStatus(id: string, paymentStatus: Order['paymentStatus']): Promise<boolean> {
  ...
  static async getStats(filters: { vendorId?: string; dateFrom?: Date; dateTo?: Date } = {}): Promise<{
    totalOrders: number;
    totalRevenue: number;
  ...
  private static async generateOrderNumber(client: PoolClient): Promise<string> {
    const prefix = 'ORD';
